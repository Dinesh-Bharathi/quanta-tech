...
//       try {
//         const res = await AuthApi.checkEmailVerification({ email });
//         if (res.status === 200 && res.data.verified) {
//           toast.success("Email verified successfully!");
//           clearInterval(interval);
//           router.push(`/signup/onboarding?user_uuid=${res.data.user_uuid}`);
//         }
//       } catch {
...
//     setIsResending(true);
//     try {
//       const res = await AuthApi.sendVerificationEmail({ email });
//       if (res.status === 200) toast.success("Verification email resent!");
//     } catch {
...
//         <div className="text-center max-w-sm mx-auto mt-10 p-6 border rounded-xl shadow-sm">
//           <MailCheck className="mx-auto mb-3 h-10 w-10 text-blue-500" />
...
const SignupForm = () => {
  ...
  const [isResending, setIsResending] = useState(false);
  const [email, setEmail] = useState("lico@putsbox.com");
